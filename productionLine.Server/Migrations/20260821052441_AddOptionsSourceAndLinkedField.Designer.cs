// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Oracle.EntityFrameworkCore.Metadata;
using productionLine.Server.Model;

#nullable disable

namespace productionLine.Server.Migrations
{
    [DbContext(typeof(FormDbContext))]
    [Migration("20260821052441_AddOptionsSourceAndLinkedField")]
    partial class AddOptionsSourceAndLinkedField
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.3")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            OracleModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("productionLine.Server.Model.AuditPlan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ApprovalComments")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<DateTime?>("ApprovedAt")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("ApprovedBy")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("ApproverAdObjectId")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("ApproverEmail")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("ApproverName")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("Description")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("DurationType")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<DateTime>("EndDate")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("PlanName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.HasKey("Id");

                    b.ToTable("AuditPlans");
                });

            modelBuilder.Entity("productionLine.Server.Model.AuditPlanEntry", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AuditPlanId")
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("AuditType")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("AuditeeEmail")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("AuditeeId")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("AuditeeName")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("AuditorEmail")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("AuditorId")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("AuditorName")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("Department")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("Frequency")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("HangfireJobId")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<int>("ReminderDaysBefore")
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("ReminderJobId")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<DateTime>("ScheduledDate")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("Scope")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("NVARCHAR2(300)");

                    b.HasKey("Id");

                    b.HasIndex("AuditPlanId");

                    b.ToTable("AuditPlanEntries");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailSchedule", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Body")
                        .IsRequired()
                        .HasColumnType("CLOB")
                        .HasColumnName("BODY")
                        .HasAnnotation("Relational:JsonPropertyName", "body");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("CREATED_AT");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("CREATED_BY");

                    b.Property<string>("CronExpression")
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("CRON_EXPRESSION")
                        .HasAnnotation("Relational:JsonPropertyName", "cronExpression");

                    b.Property<DateTime?>("EndDateTime")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("END_DATETIME")
                        .HasAnnotation("Relational:JsonPropertyName", "endDateTime");

                    b.Property<DateTime?>("LastSentAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("LAST_SENT_AT");

                    b.Property<DateTime?>("NextSendAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("NEXT_SEND_AT");

                    b.Property<string>("OccurrenceType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)")
                        .HasColumnName("OCCURRENCE_TYPE")
                        .HasAnnotation("Relational:JsonPropertyName", "occurrenceType");

                    b.Property<string>("RecurrenceDays")
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("RECURRENCE_DAYS")
                        .HasAnnotation("Relational:JsonPropertyName", "recurrenceDays");

                    b.Property<string>("SendTime")
                        .HasMaxLength(5)
                        .HasColumnType("NVARCHAR2(5)")
                        .HasColumnName("SEND_TIME")
                        .HasAnnotation("Relational:JsonPropertyName", "sendTime");

                    b.Property<DateTime>("StartDateTime")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("START_DATETIME")
                        .HasAnnotation("Relational:JsonPropertyName", "startDateTime");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)")
                        .HasColumnName("STATUS")
                        .HasAnnotation("Relational:JsonPropertyName", "status");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("NVARCHAR2(500)")
                        .HasColumnName("SUBJECT")
                        .HasAnnotation("Relational:JsonPropertyName", "subject");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("TITLE")
                        .HasAnnotation("Relational:JsonPropertyName", "title");

                    b.Property<int>("TotalSentCount")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("TOTAL_SENT_COUNT");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("UPDATED_AT");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("UPDATED_BY");

                    b.HasKey("Id");

                    b.HasIndex("CreatedBy");

                    b.HasIndex("NextSendAt");

                    b.HasIndex("Status");

                    b.ToTable("FF_EMAIL_SCHEDULE");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailScheduleAttachment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ContentType")
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("CONTENT_TYPE");

                    b.Property<int>("EmailScheduleId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("EMAIL_SCHEDULE_ID");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("NVARCHAR2(255)")
                        .HasColumnName("FILE_NAME");

                    b.Property<string>("FilePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("NVARCHAR2(500)")
                        .HasColumnName("FILE_PATH");

                    b.Property<long>("FileSizeBytes")
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("FILE_SIZE_BYTES");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("UPLOADED_AT");

                    b.HasKey("Id");

                    b.HasIndex("EmailScheduleId");

                    b.ToTable("FF_EMAIL_SCHEDULE_ATTACHMENT");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailScheduleLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("EmailScheduleId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("EMAIL_SCHEDULE_ID");

                    b.Property<string>("ErrorMessage")
                        .HasColumnType("CLOB")
                        .HasColumnName("ERROR_MESSAGE");

                    b.Property<int>("RecipientsFailed")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("RECIPIENTS_FAILED");

                    b.Property<string>("RecipientsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("RECIPIENTS_JSON");

                    b.Property<int>("RecipientsSucceeded")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("RECIPIENTS_SUCCEEDED");

                    b.Property<int>("RecipientsTotal")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("RECIPIENTS_TOTAL");

                    b.Property<DateTime>("SentAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("SENT_AT");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)")
                        .HasColumnName("STATUS");

                    b.HasKey("Id");

                    b.HasIndex("EmailScheduleId", "SentAt");

                    b.ToTable("FF_EMAIL_SCHEDULE_LOG");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailScheduleRecipient", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AdObjectId")
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("AD_OBJECT_ID");

                    b.Property<string>("Email")
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("EMAIL")
                        .HasAnnotation("Relational:JsonPropertyName", "email");

                    b.Property<int>("EmailScheduleId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("EMAIL_SCHEDULE_ID");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("NAME")
                        .HasAnnotation("Relational:JsonPropertyName", "name");

                    b.Property<string>("RecipientType")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("NVARCHAR2(10)")
                        .HasColumnName("RECIPIENT_TYPE")
                        .HasAnnotation("Relational:JsonPropertyName", "recipientType");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("NVARCHAR2(10)")
                        .HasColumnName("TYPE")
                        .HasAnnotation("Relational:JsonPropertyName", "type");

                    b.HasKey("Id");

                    b.HasIndex("EmailScheduleId");

                    b.ToTable("FF_EMAIL_SCHEDULE_RECIPIENT");
                });

            modelBuilder.Entity("productionLine.Server.Model.Form", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AllowPartialFill")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ALLOW_PARTIAL_FILL");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("CREATEDAT");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("CREATEDBY");

                    b.Property<string>("FormLink")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FORMLINK")
                        .HasAnnotation("Relational:JsonPropertyName", "formLink");

                    b.Property<string>("KeyFieldMappingsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("KEY_FIELD_MAPPINGS");

                    b.Property<int?>("LinkedFormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("LINKED_FORM_ID")
                        .HasAnnotation("Relational:JsonPropertyName", "allowedUsers");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("NAME")
                        .HasAnnotation("Relational:JsonPropertyName", "name");

                    b.Property<byte[]>("RowVersion")
                        .IsConcurrencyToken()
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("RAW(8)")
                        .HasColumnName("ROWVERSION");

                    b.HasKey("Id");

                    b.ToTable("FF_FORM");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormAccess", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AccessLevel")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ACCESSLEVEL");

                    b.Property<string>("AdObjectId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ADOBJECTID");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("EMAIL");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<int>("Level")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("LEVEL");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("NAME");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("TYPE");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_FORMACCESS");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormApproval", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<int>("ApprovalLevel")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("APPROVALLEVEL");

                    b.Property<DateTime?>("ApprovedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("APPROVALAT");

                    b.Property<int>("ApproverId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("APPROVERID");

                    b.Property<string>("ApproverName")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("APPROVERNAME");

                    b.Property<string>("Comments")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("COMMENTS");

                    b.Property<long>("FormSubmissionId")
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("FORMSUBMISSIONID");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("STATUS");

                    b.HasKey("Id");

                    b.HasIndex("FormSubmissionId");

                    b.ToTable("FF_FORMAPPROVAL");

                    b.HasAnnotation("Relational:JsonPropertyName", "approvals");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormApprover", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AdObjectId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ADOBJECTID");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("EMAIL");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<int>("Level")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("LEVEL");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("NAME");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("TYPE");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_FORMAPPROVER");

                    b.HasAnnotation("Relational:JsonPropertyName", "approvers");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormField", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("RAW(16)")
                        .HasColumnName("ID");

                    b.Property<bool?>("AllowAddRows")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ALLOW_ADD_ROWS");

                    b.Property<bool?>("AllowEditQuestions")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ALLOW_EDIT_QUESTIONS");

                    b.Property<bool?>("AllowManualEntry")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ALLOW_MANUAL_ENTRY");

                    b.Property<string>("Columns")
                        .HasColumnType("CLOB");

                    b.Property<string>("ColumnsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("COLUMNS");

                    b.Property<bool?>("Decimal")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("DECIMAL");

                    b.Property<string>("DefaultRowsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("DEFAULT_ROWS");

                    b.Property<string>("DisplayFormat")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("DISPLAY_FORMAT");

                    b.Property<string>("DisplayMode")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("DISPLAY_MODE");

                    b.Property<string>("FieldReferencesJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("FIELD_REFERENCES");

                    b.Property<string>("FilledBy")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FILLEDBY");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<string>("Formula")
                        .HasColumnType("CLOB")
                        .HasColumnName("FORMULA");

                    b.Property<string>("IMAGEOPTIONS")
                        .HasColumnType("CLOB")
                        .HasColumnName("IMAGEOPTIONS");

                    b.Property<string>("ImageValue")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("IMAGEVALUE");

                    b.Property<int?>("InitialRows")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("INITIAL_ROWS");

                    b.Property<string>("KeyFieldMappingsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("KEY_FIELD_MAPPINGS");

                    b.Property<string>("Label")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("LABEL");

                    b.Property<string>("LinkedColumnId")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("LINKED_COLUMN_ID");

                    b.Property<Guid?>("LinkedFieldId")
                        .HasColumnType("RAW(16)")
                        .HasColumnName("LINKED_FIELD_ID");

                    b.Property<string>("LinkedFieldReference")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("LINKED_FIELD_REFERENCE");

                    b.Property<string>("LinkedFieldType")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("LINKED_FIELD_TYPE");

                    b.Property<int?>("LinkedFormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("LINKED_FORM_ID");

                    b.Property<Guid?>("LinkedGridFieldId")
                        .HasColumnType("RAW(16)")
                        .HasColumnName("LINKED_GRID_FIELD_ID");

                    b.Property<double?>("Max")
                        .HasColumnType("BINARY_DOUBLE")
                        .HasColumnName("MAX");

                    b.Property<int?>("MaxRows")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("MAX_ROWS");

                    b.Property<double?>("Min")
                        .HasColumnType("BINARY_DOUBLE")
                        .HasColumnName("MIN");

                    b.Property<int?>("MinRows")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("MIN_ROWS");

                    b.Property<string>("Options")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("OptionsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("OPTIONS");

                    b.Property<string>("OptionsSource")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("OPTIONS_SOURCE");

                    b.Property<int>("Order")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ORDER");

                    b.Property<string>("RemarkTriggersJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("REMARK_TRIGGERS");

                    b.Property<bool?>("RequireRemarksOutOfRange")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("REMARKS_OUT");

                    b.Property<bool>("Required")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("REQUIRED");

                    b.Property<string>("RequiresRemarks")
                        .HasColumnType("NVARCHAR2(2000)");

                    b.Property<string>("RequiresRemarksJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("REQUIRES_REMARKS");

                    b.Property<bool?>("ResultDecimal")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("RESULT_DECIMAL");

                    b.Property<bool?>("ShowLookupButton")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("SHOW_LOOKUP_BUTTON");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("TYPE");

                    b.Property<string>("Width")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("WIDTH");

                    b.Property<string>("lengthValidationMessage")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("MESSAGE");

                    b.Property<int?>("maxLength")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("MAXLENGTH");

                    b.Property<int?>("minLength")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("MINLENGTH");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_FORMFIELD");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSignature", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<string>("FieldLabel")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FIELDLABEL");

                    b.Property<int>("FormSubmissionId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMSUBMISSIONID");

                    b.Property<long>("FormSubmissionId1")
                        .HasColumnType("NUMBER(19)");

                    b.Property<string>("SignatureData")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("SIGNATUREDATA");

                    b.Property<DateTime>("SignedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("SIGNEDAT");

                    b.HasKey("Id");

                    b.HasIndex("FormSubmissionId1");

                    b.ToTable("FF_FORMSIGNATURE");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSubmission", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "status");

                    b.Property<DateTime>("SubmittedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("SUBMITTEDAT");

                    b.Property<string>("SubmittedBy")
                        .HasColumnType("NVARCHAR2(450)")
                        .HasColumnName("SUBMITTEDBY");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.HasIndex("SubmittedBy");

                    b.HasIndex("FormId", "SubmittedAt")
                        .HasDatabaseName("IX_FORMSUBMISSIONS_FORMID_DATE");

                    b.HasIndex("FormId", "SubmittedAt", "Id")
                        .HasDatabaseName("IX_FORMSUBMISSIONS_FORMID_DATE_ID");

                    b.ToTable("FF_FORMSUBMISSION");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSubmissionData", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<string>("FieldLabel")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(450)")
                        .HasColumnName("FIELDLABEL");

                    b.Property<string>("FieldValue")
                        .IsRequired()
                        .HasColumnType("CLOB")
                        .HasColumnName("FIELDVALUE");

                    b.Property<long>("FormSubmissionId")
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("FORMSUBMISSIONID");

                    b.HasKey("Id");

                    b.HasIndex("FormSubmissionId");

                    b.HasIndex("FieldLabel", "FormSubmissionId")
                        .HasDatabaseName("IX_SUBMDATA_FIELDLABEL_SUBID");

                    b.ToTable("FF_FORMSUBMISSIONDATA");

                    b.HasAnnotation("Relational:JsonPropertyName", "submissionData");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormToAdd", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AdObjectId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ADOBJECTID");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("EMAIL");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<int>("Level")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("LEVEL");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("NAME");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("TYPE");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_FORMTOADD");
                });

            modelBuilder.Entity("productionLine.Server.Model.GridColumn", b =>
                {
                    b.Property<bool?>("AllowManualEntry")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "allowManualEntry");

                    b.Property<bool?>("Decimal")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "decimal");

                    b.Property<string>("DisplayFormat")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "displayFormat");

                    b.Property<string>("DisplayMode")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "displayMode");

                    b.Property<string>("EndTime")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "endTime");

                    b.Property<bool?>("Fixed")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "fixed");

                    b.Property<string>("Formula")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "formula");

                    b.Property<string>("Id")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "id");

                    b.Property<string>("KeyFieldMappingsJson")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "keyFieldMappingsJson");

                    b.Property<string>("Label")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "label");

                    b.Property<string>("LinkedColumnId")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "linkedColumnId");

                    b.Property<Guid?>("LinkedFieldId")
                        .HasColumnType("RAW(16)")
                        .HasAnnotation("Relational:JsonPropertyName", "linkedFieldId");

                    b.Property<string>("LinkedFieldType")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "linkedFieldType");

                    b.Property<int?>("LinkedFormId")
                        .HasColumnType("NUMBER(10)")
                        .HasAnnotation("Relational:JsonPropertyName", "linkedFormId");

                    b.Property<Guid?>("LinkedGridFieldId")
                        .HasColumnType("RAW(16)")
                        .HasAnnotation("Relational:JsonPropertyName", "linkedGridFieldId");

                    b.Property<double?>("Max")
                        .HasColumnType("BINARY_DOUBLE")
                        .HasAnnotation("Relational:JsonPropertyName", "max");

                    b.Property<double?>("Min")
                        .HasColumnType("BINARY_DOUBLE")
                        .HasAnnotation("Relational:JsonPropertyName", "min");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "name");

                    b.PrimitiveCollection<string>("Options")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "options");

                    b.Property<string>("ParentColumn")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "parentColumn");

                    b.Property<string>("RatingStyle")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "ratingStyle");

                    b.PrimitiveCollection<string>("RemarksOptions")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "remarksOptions");

                    b.Property<bool?>("Required")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "required");

                    b.Property<bool?>("ShowLookupButton")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "showLookupButton");

                    b.Property<string>("StartTime")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "startTime");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "type");

                    b.Property<string>("Width")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "width");

                    b.Property<string>("backgroundColor")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "backgroundColor");

                    b.Property<bool?>("disable")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "disabled");

                    b.Property<string>("labelStyle")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "labelStyle");

                    b.Property<string>("labelText")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "labelText");

                    b.Property<string>("lengthValidationMessage")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "lengthValidationMessage");

                    b.Property<int?>("maxLength")
                        .HasColumnType("NUMBER(10)")
                        .HasAnnotation("Relational:JsonPropertyName", "maxLength");

                    b.Property<int?>("minLength")
                        .HasColumnType("NUMBER(10)")
                        .HasAnnotation("Relational:JsonPropertyName", "minLength");

                    b.Property<string>("textAlign")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "textAlign");

                    b.Property<string>("textColor")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasAnnotation("Relational:JsonPropertyName", "textColor");

                    b.Property<bool?>("visible")
                        .HasColumnType("BOOLEAN")
                        .HasAnnotation("Relational:JsonPropertyName", "visible");

                    b.ToTable((string)null);
                });

            modelBuilder.Entity("productionLine.Server.Model.LineConfig", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("VARCHAR2(36)")
                        .HasColumnName("ID");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP")
                        .HasColumnName("CREATED_AT");

                    b.Property<string>("EngineersJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("ENGINEERS");

                    b.Property<string>("FormId")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("VARCHAR2(100)")
                        .HasColumnName("FORMID");

                    b.Property<string>("Plant")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("VARCHAR2(200)")
                        .HasColumnName("PLANT");

                    b.Property<string>("ShiftTemplateId")
                        .HasMaxLength(100)
                        .HasColumnType("VARCHAR2(100)")
                        .HasColumnName("SHIFT_TEMPLATE_ID");

                    b.Property<string>("SupervisorsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("SUPERVISORS");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TIMESTAMP")
                        .HasColumnName("UPDATED_AT");

                    b.HasKey("Id");

                    b.ToTable("FF_LINE_CONFIG", (string)null);
                });

            modelBuilder.Entity("productionLine.Server.Model.NotificationLog", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NVARCHAR2(450)")
                        .HasColumnName("ID");

                    b.Property<string>("ErrorMsg")
                        .HasMaxLength(500)
                        .HasColumnType("NVARCHAR2(500)")
                        .HasColumnName("ERROR_MSG");

                    b.Property<string>("LineId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("LINE_ID");

                    b.Property<string>("LinePlant")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("LINE_PLANT");

                    b.Property<string>("Platform")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)")
                        .HasColumnName("PLATFORM");

                    b.Property<string>("RecipientId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("RECIPIENT_ID");

                    b.Property<string>("RecipientName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("RECIPIENT_NAME");

                    b.Property<DateTime>("SentAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("SENT_AT");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)")
                        .HasColumnName("STATUS");

                    b.Property<string>("SuppressReason")
                        .HasMaxLength(100)
                        .HasColumnType("NVARCHAR2(100)")
                        .HasColumnName("SUPPRESS_REASON");

                    b.HasKey("Id");

                    b.ToTable("FF_NOTIFICATION_LOG", (string)null);
                });

            modelBuilder.Entity("productionLine.Server.Model.PartialSubmission", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<string>("AssignedToEmail")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ASSIGNEDTOEMAIL");

                    b.Property<string>("AssignedToName")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ASSIGNEDTONAME");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("COMPLETEDAT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("CREATEDAT");

                    b.Property<string>("FilledBy")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FILLEDBY");

                    b.Property<string>("FilledDataJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("FILLEDDATA");

                    b.Property<string>("FilledFieldsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("FILLEDFIELDS");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("STATUS");

                    b.Property<long?>("SubmissionId")
                        .HasColumnType("NUMBER(19)")
                        .HasColumnName("SUBMISSIONID");

                    b.Property<string>("Token")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("TOKEN");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_PARTIALSUBMISSION");
                });

            modelBuilder.Entity("productionLine.Server.Model.QuietHoursConfig", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NVARCHAR2(450)")
                        .HasColumnName("ID");

                    b.Property<bool>("Enabled")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ENABLED");

                    b.Property<string>("End")
                        .HasMaxLength(5)
                        .HasColumnType("NVARCHAR2(5)")
                        .HasColumnName("END_TIME");

                    b.Property<bool>("SkipBreaks")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("SKIP_BREAKS");

                    b.Property<string>("Start")
                        .HasMaxLength(5)
                        .HasColumnType("NVARCHAR2(5)")
                        .HasColumnName("START_TIME");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("UPDATED_AT");

                    b.HasKey("Id");

                    b.ToTable("FF_QUIET_HOURS", (string)null);
                });

            modelBuilder.Entity("productionLine.Server.Model.RecipientConfig", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NVARCHAR2(450)")
                        .HasColumnName("ID");

                    b.Property<bool>("Android")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ANDROID");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("CREATED_AT");

                    b.Property<int>("DelayMin")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("DELAY_MIN");

                    b.Property<string>("DeviceTokensJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("DEVICE_TOKENS");

                    b.Property<string>("Email")
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("EMAIL");

                    b.Property<bool>("Enabled")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("ENABLED");

                    b.Property<bool>("Ios")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("IOS");

                    b.Property<string>("LineIdsJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("LINE_IDS");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("NVARCHAR2(200)")
                        .HasColumnName("NAME");

                    b.Property<string>("Phone")
                        .HasMaxLength(50)
                        .HasColumnType("NVARCHAR2(50)")
                        .HasColumnName("PHONE");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("UPDATED_AT");

                    b.HasKey("Id");

                    b.ToTable("FF_RECIPIENT_CONFIG", (string)null);
                });

            modelBuilder.Entity("productionLine.Server.Model.RemarkTrigger", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<Guid>("FormFieldId")
                        .HasColumnType("RAW(16)")
                        .HasColumnName("FORMFIELDID");

                    b.Property<string>("Operator")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("OPERATOR");

                    b.Property<double>("Value")
                        .HasColumnType("BINARY_DOUBLE")
                        .HasColumnName("VALUE");

                    b.HasKey("Id");

                    b.HasIndex("FormFieldId");

                    b.ToTable("FF_REMARK_TRIGGER");
                });

            modelBuilder.Entity("productionLine.Server.Model.Report", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("CREATEDAT");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("CREATEDBY");

                    b.Property<string>("DefinitionJson")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("DEFINITIONJSON");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("DESCRIPTION");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<string>("LayoutType")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("LAYOUTTYPE");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("NAME");

                    b.PrimitiveCollection<string>("SharedWithRoles")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("SHAREDWITHROLES");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("UPDATEDAT");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_REPORT");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportAccess", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AccessType")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("ACCESSTYPE");

                    b.Property<int>("ReportId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("REPORTID");

                    b.Property<string>("UserOrGroupId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("USERGROUPID");

                    b.HasKey("Id");

                    b.HasIndex("ReportId");

                    b.ToTable("FF_REPORT_ACCESS");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportField", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("FieldId")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FIELDID");

                    b.Property<string>("FieldLabel")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FIELDLABEL");

                    b.Property<int?>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<int>("Order")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ORDER");

                    b.Property<int>("ReportTemplateId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("TEMPLATEID");

                    b.Property<bool>("Visible")
                        .HasColumnType("NUMBER(1)")
                        .HasColumnName("VISIBLE");

                    b.HasKey("Id");

                    b.HasIndex("ReportTemplateId");

                    b.ToTable("FF_REPORTFIELD");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportFilter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("FieldLabel")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FIELDLABEL");

                    b.Property<string>("Operator")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("OPERATOR");

                    b.Property<int>("ReportTemplateId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("TEMPLATEID");

                    b.Property<string>("Type")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("TYPE");

                    b.Property<string>("Value")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("VALUE");

                    b.HasKey("Id");

                    b.HasIndex("ReportTemplateId");

                    b.ToTable("FF_REPORTFILTER");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportTemplate", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ID");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CalculatedFields")
                        .HasColumnType("CLOB")
                        .HasColumnName("CALCULATEDFIELDS");

                    b.Property<string>("ChartConfig")
                        .HasColumnType("CLOB")
                        .HasColumnName("CHARTCONFIG");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("CREATEDAT");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("CREATEDBY");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("DELETEDAT");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("DELETEDBY");

                    b.Property<int>("FormId")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("FORMID");

                    b.Property<string>("FormIds")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FORMIDS");

                    b.Property<string>("FormRelationships")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("FORMRELATIONSHIPS");

                    b.Property<string>("GroupingConfig")
                        .HasColumnType("CLOB")
                        .HasColumnName("GROUPINGCONFIG");

                    b.Property<int>("IncludeApprovals")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("INCLUDEAPPROVALS");

                    b.Property<int>("IncludeRemarks")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("INCLUDEREMARKS");

                    b.Property<int>("IsDeleted")
                        .HasColumnType("NUMBER(10)")
                        .HasColumnName("ISDELETE");

                    b.Property<bool>("IsMultiForm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(1)")
                        .HasDefaultValue(false)
                        .HasColumnName("ISMULTIFORM");

                    b.Property<string>("LayoutMode")
                        .HasMaxLength(20)
                        .HasColumnType("NVARCHAR2(20)")
                        .HasColumnName("LAYOUTMODE");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("NAME");

                    b.Property<string>("SharedWithRole")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("SHAREDWITHROLE");

                    b.Property<string>("SubmittedViewers")
                        .HasColumnType("NVARCHAR2(2000)")
                        .HasColumnName("SUBMITTEDVIEWERS");

                    b.HasKey("Id");

                    b.HasIndex("FormId");

                    b.ToTable("FF_REPORTTEMPLATE");
                });

            modelBuilder.Entity("productionLine.Server.Model.ShiftConfig", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NVARCHAR2(450)")
                        .HasColumnName("ID");

                    b.Property<string>("BreaksJson")
                        .HasColumnType("CLOB")
                        .HasColumnName("BREAKS");

                    b.Property<string>("End")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("NVARCHAR2(5)")
                        .HasColumnName("END_TIME");

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("NVARCHAR2(10)")
                        .HasColumnName("SHIFT_KEY");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("NVARCHAR2(50)")
                        .HasColumnName("NAME");

                    b.Property<string>("Start")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("NVARCHAR2(5)")
                        .HasColumnName("START_TIME");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TIMESTAMP(7)")
                        .HasColumnName("UPDATED_AT");

                    b.HasKey("Id");

                    b.ToTable("FF_SHIFT_CONFIG", (string)null);
                });

            modelBuilder.Entity("productionLine.Server.Model.AuditPlanEntry", b =>
                {
                    b.HasOne("productionLine.Server.Model.AuditPlan", "AuditPlan")
                        .WithMany("Entries")
                        .HasForeignKey("AuditPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AuditPlan");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailScheduleAttachment", b =>
                {
                    b.HasOne("productionLine.Server.Model.EmailSchedule", "EmailSchedule")
                        .WithMany("Attachments")
                        .HasForeignKey("EmailScheduleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("EmailSchedule");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailScheduleLog", b =>
                {
                    b.HasOne("productionLine.Server.Model.EmailSchedule", "EmailSchedule")
                        .WithMany("Logs")
                        .HasForeignKey("EmailScheduleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("EmailSchedule");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailScheduleRecipient", b =>
                {
                    b.HasOne("productionLine.Server.Model.EmailSchedule", "EmailSchedule")
                        .WithMany("Recipients")
                        .HasForeignKey("EmailScheduleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("EmailSchedule");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormAccess", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany("AllowedUsers")
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormApproval", b =>
                {
                    b.HasOne("productionLine.Server.Model.FormSubmission", "FormSubmission")
                        .WithMany("Approvals")
                        .HasForeignKey("FormSubmissionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("FormSubmission");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormApprover", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany("Approvers")
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormField", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany("Fields")
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSignature", b =>
                {
                    b.HasOne("productionLine.Server.Model.FormSubmission", "FormSubmission")
                        .WithMany()
                        .HasForeignKey("FormSubmissionId1")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("FormSubmission");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSubmission", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany()
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSubmissionData", b =>
                {
                    b.HasOne("productionLine.Server.Model.FormSubmission", "FormSubmission")
                        .WithMany("SubmissionData")
                        .HasForeignKey("FormSubmissionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("FormSubmission");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormToAdd", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany("AllowedtoAccess")
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.PartialSubmission", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany()
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.RemarkTrigger", b =>
                {
                    b.HasOne("productionLine.Server.Model.FormField", "FormField")
                        .WithMany("RemarkTriggers")
                        .HasForeignKey("FormFieldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("FormField");
                });

            modelBuilder.Entity("productionLine.Server.Model.Report", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany()
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportAccess", b =>
                {
                    b.HasOne("productionLine.Server.Model.Report", "Report")
                        .WithMany("AccessList")
                        .HasForeignKey("ReportId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Report");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportField", b =>
                {
                    b.HasOne("productionLine.Server.Model.ReportTemplate", "ReportTemplate")
                        .WithMany("Fields")
                        .HasForeignKey("ReportTemplateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReportTemplate");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportFilter", b =>
                {
                    b.HasOne("productionLine.Server.Model.ReportTemplate", "ReportTemplate")
                        .WithMany("Filters")
                        .HasForeignKey("ReportTemplateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReportTemplate");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportTemplate", b =>
                {
                    b.HasOne("productionLine.Server.Model.Form", "Form")
                        .WithMany()
                        .HasForeignKey("FormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Form");
                });

            modelBuilder.Entity("productionLine.Server.Model.AuditPlan", b =>
                {
                    b.Navigation("Entries");
                });

            modelBuilder.Entity("productionLine.Server.Model.EmailSchedule", b =>
                {
                    b.Navigation("Attachments");

                    b.Navigation("Logs");

                    b.Navigation("Recipients");
                });

            modelBuilder.Entity("productionLine.Server.Model.Form", b =>
                {
                    b.Navigation("AllowedUsers");

                    b.Navigation("AllowedtoAccess");

                    b.Navigation("Approvers");

                    b.Navigation("Fields");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormField", b =>
                {
                    b.Navigation("RemarkTriggers");
                });

            modelBuilder.Entity("productionLine.Server.Model.FormSubmission", b =>
                {
                    b.Navigation("Approvals");

                    b.Navigation("SubmissionData");
                });

            modelBuilder.Entity("productionLine.Server.Model.Report", b =>
                {
                    b.Navigation("AccessList");
                });

            modelBuilder.Entity("productionLine.Server.Model.ReportTemplate", b =>
                {
                    b.Navigation("Fields");

                    b.Navigation("Filters");
                });
#pragma warning restore 612, 618
        }
    }
}
