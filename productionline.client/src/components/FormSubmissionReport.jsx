...
function SubmissionSearchBox({ value, onChange, onClear }) {
    return (
        <div className="relative w-64">
            ...
            <input
                ...
                className="w-full pl-9 pr-8 py-1.5 text-sm border border-gray-300 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400"
            />
            {value && (
                <button
                    onClick={onClear}
...
export default function FormSubmissionReport() {
    ...
    // Filter submissions by search ID (client-side, on current page)
    const filteredSubmissions = searchId.trim()
        ? submissions.filter(s =>
            String(s.id).toLowerCase().includes(searchId.trim().toLowerCase())
        )
        : submissions;
            ...
            {loading ? (
                ...
                <div>
                    ...
                    {filteredSubmissions.length === 0 ? (
                        <p className="text-gray-500">{searchId ? `No submissions matching "${searchId}".` : "No submissions found."}</p>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="min-w-full bg-white border border-gray-200">
                                <thead className="bg-gray-100">
                            ...
                            {/* Hide pagination when actively searching — results are already filtered */}
                            {!searchId && (
                                <Pagination
                                    page={page}
                                    totalPages={totalPages}
                ...
                <div>
                    {renderTableHeader()}
                    {groupSubmissionsBySubmissionId().length === 0 ? (
                        <p className="text-gray-500">{searchId ? `No submissions matching "${searchId}".` : "No submissions found."}</p>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="min-w-full bg-white border border-gray-200">
                            ...
                            {/* Hide pagination when actively searching */}
                            {!searchId && (
                                <Pagination
                                    page={page}
                                    totalPages={totalPages}
